<template>
  <div class="select-condition">
              ...
              <div class="plist">
                ...
                <template>
                  <span  v-if="tempaltes.selectedOptions.category.first">
                    {{tempaltes.selectedOptions.category.first.name}}
                    <i
                    class="iconfont icon-cuo"
    ...
    <!-- <button @click="delSelect('whereHouse')">测试发货地</button>
    <button @click="delSelect('brand')">测试品牌</button>
    <button @click="delSelect('property', 1002)">测试品牌</button> -->
  </div>
</template>
...
${this.config.keyword !== ''?'keyword='+this.config.keyword:''}
${this.config.firstCategory !== ''?'&firstCategory='+this.config.firstCategory:0}
${this.config.secondCategory !== ''?'&secondCategory='+this.config.secondCategory:0}
${this.config.thirdCategory !== ''?'&thirdCategory='+this.config.thirdCategory:0}
${this.config.brandId !== ''?'&brandId='+this.config.brandId:0}
${this.config.orderType !== ''?'&orderType='+this.config.orderType:1}
${this.config.pageNo !== ''?'&pageNo='+this.config.pageNo:1}
${this.config.pageSize !== ''?'&pageSize='+this.config.pageSize:40}
${this.config.price !== ''?'&price='+this.config.price:0}
${this.config.prop !== ''?'&prop='+this.config.prop:0}
${this.config.whereHouse !== ''?'&whereHouse='+this.config.whereHouse:0}
${this.config.source !== ''?'&source='+this.config.source:0}
`)
    }
